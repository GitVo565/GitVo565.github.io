...
When you **update** an HTML page, include in the `<title>` a **3-character alphanumeric version** so that when sources are imported to an LM and sorted by title, older vs newer versions are clear. **Ordering:** `0` through `9`, then `a` through `z` (e.g. `000` → `001` → … → `009` → `00a` → … → `00z` → `010` → … → `zzz`). **Format:** Append ` · vXXX` to the title (e.g. `00W38 — Donald's Venture · v001`). On each subsequent update of that page, **increment** the version to the next value in sort order (e.g. `v001` → `v002` → … → `v009` → `v00a` → … → `v00z` → `v010`). New pages get `v000` or `v001`. This way, sorting titles alphanumerically (0→9→a→z) gives oldest to newest when multiple versions are in an LM’s source set.
